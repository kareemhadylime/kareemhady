// Pure URL-state helpers for the Performance (P&L) page.
//
// Kept in a non-`'use client'` file so server components (e.g.
// `financials/performance/page.tsx`) can import + call `parseFinPerfState`
// directly. Next.js 16 treats every export from a `'use client'` module as
// a client reference and refuses to invoke it from the server — moving the
// pure logic here avoids that boundary error. The React hook
// (`usePerfPnlUrlState`) lives in the sibling `use-perf-pnl-url-state.ts`.
...
export const PERF_PNL_BASE_PATH = '/beithady/financials/performance';

export const PERF_PNL_DEFAULTS: FinPerfUrlState = {
  scope: 'consolidated',
