  ...
  return (
    ...
    >
      ...
      <section className="flex items-center gap-2 flex-wrap text-xs">
        <StatusChip label="All" href="/beithady/inventory/vendors" active={!sp.status || sp.status === 'all'} count={vendors.length} tone="neutral" />
      ...
      <section className="flex items-center justify-between gap-3 flex-wrap">
        <form action="" method="get" className="flex items-center gap-2 flex-wrap">
          ...
          {(sp.search || sp.category) && (
            <Link href={`/beithady/inventory/vendors${sp.status ? `?status=${sp.status}` : ''}`} className="text-[11px] text-slate-500 hover:text-slate-700">Clear</Link>
          )}
        </form>
