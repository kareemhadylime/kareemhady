...
export async function addRecipientFormAction(formData: FormData): Promise<void> {
  ...
  if (!r.ok) {
    redirect(`/beithady/setup?err=${encodeURIComponent(r.error)}`);
  }
}
...
export async function sendTestNowFormAction(): Promise<void> {
  ...
  if (!r.ok) {
    ...
    redirect(
      `/beithady/setup?test=${r.sent}sent_${r.failed}failed&link=${encodeURIComponent(r.preview_link)}`
    );
  }
}
