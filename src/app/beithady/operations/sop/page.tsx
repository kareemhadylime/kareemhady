  ...
  return (
    <BeithadyShell breadcrumbs={[
      ...
      <section className="ix-card p-2 flex flex-wrap items-center gap-2 text-xs">
        ...
        <Link
          href={role ? `?role=${role}${sub ? `&sub=${sub}` : ''}${search ? `&q=${search}` : ''}` : '/beithady/operations/sop'}
        ...
        <form action="/beithady/operations/sop" className="ml-auto flex items-center gap-1">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {articles.map(a => {
            ...
            return (
              <Link
                key={a.id}
                href={`/beithady/operations/sop/${a.slug}`}
