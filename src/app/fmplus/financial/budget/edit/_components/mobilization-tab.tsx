    ...
    <div className="space-y-3">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
        <div className="flex justify-between items-center mb-3 flex-wrap gap-2">
          <div>
            <strong className="text-sm text-slate-900 dark:text-slate-100">Mobilization · {contractName}</strong>
        ...
        {rows.length === 0 ? (
          ...
          <div className="space-y-2">
            {rows.map((r, idx) => {
              ...
              return (
                <div key={idx} className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded p-3 space-y-2">
                  <div className="flex flex-wrap gap-2 items-end">
                    <label className="block">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase block">Category</span>
                      <select value={r.category}
                        onChange={e => updateRow(idx, { category: e.currentTarget.value as MobCategory })}
                        disabled={!canEdit || isPending}
                        className="text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded px-2 py-1 disabled:opacity-50">
                    ...
                    <label className="block flex-1 min-w-[200px]">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase block">Label (English)</span>
                      <input value={r.label_en}
                        ...
                        className="w-full text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded px-2 py-1 disabled:opacity-50" />
                    </label>
                    <label className="block">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase block">Qty</span>
                      <input type="number" min="0" step="0.01" value={r.qty}
                        onChange={e => updateRow(idx, { qty: Number(e.currentTarget.value) || 0 })}
                        disabled={!canEdit || isPending}
                        className="w-20 px-2 py-1 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-right tabular-nums disabled:opacity-50" />
                    </label>
                    <label className="block">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase block">Unit cost</span>
                      <input type="number" min="0" step="0.01" value={r.unit_cost}
                        onChange={e => updateRow(idx, { unit_cost: Number(e.currentTarget.value) || 0 })}
                        disabled={!canEdit || isPending}
                        className="w-28 px-2 py-1 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-right tabular-nums disabled:opacity-50" />
                    </label>
                    <button type="button" onClick={() => removeRow(idx)}
                      disabled={!canEdit || isPending}
                      className="text-slate-500 dark:text-slate-400 hover:text-red-500 mb-1 disabled:opacity-50"
                      title="Remove line">
                      <Trash2 size={14} />
                    </button>
                  </div>
                  <div className="flex flex-wrap gap-2 items-end">
                    <label className="block">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase block">Amortization</span>
                    ...
                    <label className="block">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase block">Months</span>
                    ...
                    <label className="block flex-1 min-w-[150px]">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase block">Label (Arabic)</span>
                    ...
                    <div className="text-[10px] text-slate-500 dark:text-slate-400 tabular-nums text-right ml-auto">
