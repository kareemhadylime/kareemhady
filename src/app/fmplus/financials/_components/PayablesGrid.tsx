import { Wrench, Landmark, HandCoins } from 'lucide-react';
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <Card
        ...
        subtitle="Suppliers, subcontractors, accrued expenses"
        Icon={Wrench}
        tone="amber"
        bucket={report.vendors}
        valueLabel="Net outstanding (residual amount, EGP)"
        unitLabel="partners"
      />
      <Card
        title="Government Payables"
        subtitle="Withholding tax, social insurance, levies"
        Icon={Landmark}
        tone="rose"
        bucket={report.government}
        valueLabel="Net outstanding (residual amount, EGP)"
        unitLabel="partners"
      />
      <Card
        title="Customer Receivables"
        subtitle="Open AR — invoiced but unpaid"
        Icon={HandCoins}
        tone="emerald"
        bucket={report.receivables}
        valueLabel="Outstanding (residual amount, EGP)"
        unitLabel="customers"
      />
    </section>
  );
...
function Card({ title, subtitle, Icon, tone, bucket, valueLabel, unitLabel }: {
  title: string;
  subtitle: string;
  Icon: LucideIcon;
  tone: Tone;
    ...
    <div className={`group relative ix-card p-5 overflow-hidden flex flex-col transition border ${t.border}`}>
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${t.badge}`}>
          {bucket.partners.length} {bucket.partners.length === 1 ? unitLabel.replace(/s$/, '') : unitLabel}
        </span>
      </div>

      <p className={`text-3xl font-bold tabular-nums mt-4 ${valueClr}`}>{fmt(bucket.total)}</p>
      <p className="text-[11px] text-slate-500 dark:text-slate-400">{valueLabel}</p>

      <div className="mt-3 -mx-2 flex-1 min-h-0">
        {bucket.partners.length === 0 ? (
