  ...
  return (
    <section>
      <h2 className="text-base font-semibold tracking-tight font-serif mb-3 flex items-center gap-2">
        <AlertTriangle size={16} className="text-orange-300" /> Needs Attention
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {rows.slice(0, 6).map(r => (
          <Link key={r.contract_id} href={r.drill_url} className="ix-card p-4 hover:shadow-lg transition">
            ...
            <p className={`text-2xl font-bold tabular-nums mt-2 ${r.health === 'bad' ? 'text-red-300' : 'text-orange-300'}`}>{(r.variance_pct * 100).toFixed(1)}%</p>
