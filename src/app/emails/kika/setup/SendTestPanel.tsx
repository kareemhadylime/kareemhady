...
// Client-side "Send Test Report Now" button. React 19 useActionState gives:
//   1. Idle → click triggers immediate Sending… with spinner
//   2. Pending → button disabled, message says it can take 30–60s
    ...
    <div className="mt-5 border-t border-slate-200 dark:border-slate-700 pt-4">
      <form action={formAction} className="flex items-center gap-3 flex-wrap">
        <button
          type="submit"
          disabled={isPending}
          className="inline-flex items-center gap-2 rounded-md bg-[#0F172A] dark:bg-[#0F172A] px-4 py-2 text-sm font-semibold text-white hover:bg-[#1E293B] disabled:opacity-60 disabled:cursor-not-allowed transition"
        >
          {isPending ? (
            <>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Builds today&apos;s report and delivers to recipients matching your
          username/whatsapp (or all active if none match). Skips the 9 AM
          gate. Takes 30–60s.
        </p>
      </form>
      ...
      {isPending && (
        ...
        >
          ...
          <div>
            <strong>Processing…</strong> loading 60-day Shopify corpus, running
            section builders (topline + products + inventory + abandoned +
            fulfillment + discounts + geo), rendering the PDF, and pushing to
            WhatsApp + Email. Please don&apos;t close this tab.
          </div>
        </div>
      )}
