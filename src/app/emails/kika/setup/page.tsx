...
import { Mail, MessageCircle, Trash2, Power, ChevronRight, Calendar } from 'lucide-react';
...
export default async function KikaSetupPage({
  searchParams,
}: {
  ...
  if (today) {
    const { data: dels } = await sb
      ...
      .select(
        'channel, destination, status, error, sent_at, attempt, snapshot:daily_report_snapshots!inner(report_date, report_kind)'
      )
      .eq('snapshot.report_date', today)
      .eq('snapshot.report_kind', REPORT_KIND)
      .order('sent_at', { ascending: false })
      .limit(50);
    todayDeliveries = ((dels as unknown as Delivery[] | null) || []).map(d => ({
      channel: d.channel,
    ...
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="mx-auto max-w-5xl px-4 py-8">
        ...
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">
          KIKA Daily Report Setup
        </h1>
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
          X-Label · KIKA daily performance digest. Recipients + delivery
          monitoring. Admin-only.
        </p>

        <div className="mt-3">
          <Link
            href="/emails/kika/history"
            className="inline-flex items-center gap-1.5 text-xs font-medium text-pink-700 dark:text-pink-400 hover:underline"
          >
            <Calendar className="h-3 w-3" /> View 90-day snapshot history →
          </Link>
        </div>

        {/* Recipients */}
        <section className="mt-6 ix-card p-5">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-pink-700 dark:text-pink-400">
            Daily Report Recipients
          </h2>
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Sent at 09:00 Cairo daily. WhatsApp gets a tokenized HTML link;
            Email gets the link plus an A4 PDF attachment. Phone numbers must
            include{' '}
            <code className="px-1 rounded bg-slate-100 dark:bg-slate-800">+</code>{' '}
          ...
          >
            ...
            <button
              type="submit"
              className="md:col-span-4 rounded-md bg-[#0F172A] px-4 py-2 text-sm font-semibold text-white hover:bg-[#1E293B]"
            >
              + Add recipient
            </button>
          </form>
                  ...
                  >
                    <td className="py-2 pr-4">
                      {r.channel === 'whatsapp' ? (
                        ...
                        <span className="inline-flex items-center gap-1 rounded bg-pink-50 dark:bg-pink-950 px-2 py-0.5 text-xs font-medium text-pink-700 dark:text-pink-300">
                ...
                {snapshots.map(s => (
                  ...
                  >
                    ...
                    <td className="py-2 pr-4 text-xs text-slate-500 dark:text-slate-400">
                      {fmtCairoDateTime(s.generated_at)}
                    </td>
                    <td className="py-2 pr-4">
                      {s.delivery_complete ? (
                    ...
                    <td className="py-2 pr-4 text-xs text-rose-700 dark:text-rose-300">
                      {s.last_build_error
                        ? s.last_build_error.slice(0, 60) +
                          (s.last_build_error.length > 60 ? '…' : '')
                        : '—'}
                    </td>
                    <td className="py-2">
                      <Link
                        href={`/r/kika/${encodeURIComponent(s.token)}`}
                        target="_blank"
                        className="text-xs font-medium text-pink-700 dark:text-pink-400 hover:underline"
                      >
                        View →
                      </Link>
                    </td>
              ...
              <tbody>
                {todayDeliveries.map((d, i) => (
                  ...
                  >
                    ...
                    <td className="py-2 pr-4">
                      {d.status === 'sent' ? (
                        <span className="rounded bg-emerald-100 dark:bg-emerald-900 px-2 py-0.5 text-xs text-emerald-800 dark:text-emerald-200">
                          sent
                        </span>
                      ) : d.status === 'failed' ? (
                        <span className="rounded bg-rose-100 dark:bg-rose-950 px-2 py-0.5 text-xs text-rose-800 dark:text-rose-200">
                          failed
                        </span>
                      ) : (
                        <span className="rounded bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-xs text-slate-600 dark:text-slate-300">
                          {d.status}
                        </span>
                      )}
                    </td>
