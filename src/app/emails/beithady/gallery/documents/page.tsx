...
export default async function DocumentsPage({
  ...
  return (
    <BeithadyShell breadcrumbs={[
      ...
      {asset && <AssetDetailModal asset={asset} closeHref="/emails/beithady/gallery/documents" />}
      ...
      <section className="space-y-2">
        <div className="flex items-center gap-2 text-xs">
          ...
          <Link href="/emails/beithady/gallery/documents" className={`px-2 py-0.5 rounded ${!sp.building ? 'bg-slate-700 text-white' : 'bg-slate-100 dark:bg-slate-800 hover:bg-slate-200'}`}>All</Link>
          {(['BH-26','BH-73','BH-435','BH-OK','BH-34'] as const).map(b => (
            <Link
              key={b}
              href={`/emails/beithady/gallery/documents?building=${b}`}
                ...
                {list.rows.map((a, i) => (
                  <tr key={a.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-stone-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="py-2 px-4 text-xs space-x-2">
                      ...
                      <Link href={`/emails/beithady/gallery/documents?asset=${a.id}${sp.building ? `&building=${sp.building}` : ''}`} className="ix-link">Details</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
