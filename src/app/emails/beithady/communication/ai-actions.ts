...
export async function acceptSuggestionAction(formData: FormData): Promise<void> {
  ...
  if (!outboundId) {
    ...
    redirect(`/emails/beithady/communication/${l.channel === 'guesty' ? 'guesty' : 'wa-casual'}?c=${conversationId}&send_error=${encodeURIComponent(errMsg || 'unknown')}`);
  }
  ...
  redirect(`/emails/beithady/communication/${l.channel === 'guesty' ? 'guesty' : 'wa-casual'}?c=${conversationId}&sent=1`);
}
...
export async function revertAutoSendAction(formData: FormData): Promise<void> {
  ...
  revalidatePath('/emails/beithady/settings/audit');
}
