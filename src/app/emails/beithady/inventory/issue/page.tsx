      ...
      <section className="flex items-center justify-between gap-3 flex-wrap">
        <form action="" method="get" className="flex items-center gap-2 flex-wrap">
          ...
          {(sp.search || sp.status || sp.type) && (
            <Link href="/emails/beithady/inventory/issue" className="text-[11px] text-slate-500 hover:text-slate-700">Clear</Link>
          )}
        </form>
        {canWrite && (
          <Link href="/emails/beithady/inventory/issue/new" className="px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-600 text-white hover:bg-emerald-700 inline-flex items-center gap-1.5 shadow-sm">
                ...
                {issues.map(i => (
                  <tr key={i.id} className="border-t border-slate-100 hover:bg-slate-50">
                    <td className="px-3 py-2 font-mono text-[11px]">
                      <Link href={`/emails/beithady/inventory/issue/${i.id}`} className="hover:text-cyan-700 hover:underline">{i.issue_no}</Link>
                    </td>
                    ...
                    <td className="px-3 py-2 text-right">
                      <Link href={`/emails/beithady/inventory/issue/${i.id}`} className="text-cyan-700 hover:text-cyan-900 inline-flex items-center gap-0.5 text-[11px]">
