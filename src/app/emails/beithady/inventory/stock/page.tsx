...
export default async function InventoryStockPage({
  ...
  return (
    ...
    >
      ...
      <section className="ix-card p-3 flex items-center gap-2 flex-wrap text-xs">
        <form action="" method="get" className="flex items-center gap-2 flex-wrap w-full">
          ...
          {(sp.search || sp.warehouse || sp.building || sp.category || sp.status) && (
            <Link href="/emails/beithady/inventory/stock" className="text-[11px] text-slate-500 hover:text-slate-700">Clear</Link>
          )}
        </form>
      </section>
      ...
      {sp.drilldown && (
        <LedgerDrawer itemId={sp.drilldown} closeHref={`/emails/beithady/inventory/stock${(() => {
          const cleaned = { ...sp };
