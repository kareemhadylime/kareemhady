...
export function GrnDraftForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        router.push(`/emails/beithady/inventory/grn/${res.grn_id}`);
      } else {
        setError(res.error);
      }
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 text-xs">
      ...
      <div className="flex items-center justify-end gap-2">
        <a href="/emails/beithady/inventory/grn" className="px-3 py-1.5 text-[11px] text-slate-500 hover:text-slate-700">Cancel</a>
