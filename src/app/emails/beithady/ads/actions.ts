...
export async function generateAdCopyAction(formData: FormData): Promise<void> {
  ...
  revalidatePath('/emails/beithady/ads/create');
  // Round-trip the IDs so the wizard can pre-populate the variant chooser
  redirect(`/emails/beithady/ads/create?copy=${ids.join(',')}&building=${buildingCode || ''}&country=${targetCountry || ''}&language=${language}`);
}
...
export async function publishCampaignAction(formData: FormData): Promise<void> {
  ...
  if (!campaignName || !buildingCodes.length || !targetCountries.length || !headline || !primaryText) {
    redirect('/emails/beithady/ads/create?error=missing_required_fields');
  }
  if (!Number.isFinite(dailyBudgetUsd) || dailyBudgetUsd < 1) {
    redirect('/emails/beithady/ads/create?error=invalid_budget');
  }
  ...
  if (!result.ok) {
    ...
    redirect(`/emails/beithady/ads/create?error=${encodeURIComponent(`${result.step}: ${result.error}`)}`);
  }

  revalidatePath('/emails/beithady/ads');
  revalidatePath('/emails/beithady/ads/campaigns');
  redirect(`/emails/beithady/ads/campaigns/${result.campaign_id}?published=${result.mode}`);
}
...
export async function setCampaignStatusAction(formData: FormData): Promise<void> {
  ...
  revalidatePath('/emails/beithady/ads');
  revalidatePath(`/emails/beithady/ads/campaigns/${campaignId}`);
}
