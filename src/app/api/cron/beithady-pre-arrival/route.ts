...
import { runPreArrivalDispatch } from '@/lib/beithady/engagement/pre-arrival';
...
function checkAuth(req: NextRequest): boolean {
  ...
  if (!expected) {
    console.error('[cron beithady-pre-arrival] CRON_SECRET unset — refusing');
    return false;
  }
...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await runPreArrivalDispatch();
