...
// the latest price from each held instrument's price_url page (currently
// english.mubasher.info — investing.com 403s from Vercel IPs) and inserts one
// row per instrument into personal_stock_current_prices.
//
// Held = v_personal_stock_positions.qty_held > 0. Skips any instrument without
// a price_url. Uses ?force=1 to bypass the Cairo-hour-17 + weekday gate when
// manually testing.
//
...
const PRICE_RE = /market-summary__last-price[^"]*">([0-9.,]+)/;
...
async function fetchPrice(url: string): Promise<number> {
  ...
  if (!m) throw new Error('last-price element not found');
  ...
  for (const ins of instruments.data ?? []) {
    if (!ins.price_url) {
      results.push({
        ticker: ins.ticker,
        instrumentId: ins.id,
        error: 'no price_url set',
      });
      continue;
    }
    try {
      const price = await fetchPrice(ins.price_url);
      inserts.push({
