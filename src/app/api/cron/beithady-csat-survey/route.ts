...
import { runCsatDispatch } from '@/lib/beithady/engagement/csat';
...
function checkAuth(req: NextRequest): boolean {
  ...
  if (!expected) {
    console.error('[cron beithady-csat-survey] CRON_SECRET unset — refusing');
    return false;
  }
...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await runCsatDispatch();
