...
export type RuleAction = { type: 'shopify_order_aggregate'; currency?: string };
...
export async function evaluateRule(ruleId: string) {
  ...
  const sinceIso = new Date(Date.now() - (cond.time_window_hours || 24) * 3600 * 1000).toISOString();
  ...
  const { data: matches, error: mErr } = await q.order('received_at', { ascending: false }).limit(500);
    ...
    if (!matches?.length) {
      await sb
        ...
        .update({
          ...
          status: 'succeeded',
          output: { order_count: 0, total_amount: 0, currency: action.currency || 'EGP', products: [], orders: [], parse_errors: 0 },
        })
        .eq('id', run.id);
